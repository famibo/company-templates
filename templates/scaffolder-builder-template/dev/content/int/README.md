# ${{ values.templateName }} - Integration Stage

This is the **integration** version of the `${{ values.templateName }}` template.
...
```bash
./scripts/promote-template.sh --template ${{ values.templateName }} --from int --to prod
```
