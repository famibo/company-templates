# ${{ values.templateName }} - Development Stage

This is the **development** version of the `${{ values.templateName }}` template.
...
```bash
./scripts/promote-template.sh --template ${{ values.templateName }} --from dev --to int
```
